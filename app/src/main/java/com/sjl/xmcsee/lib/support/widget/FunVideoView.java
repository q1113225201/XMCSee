package com.sjl.xmcsee.lib.support.widget;
...
import com.sjl.xmcsee.lib.support.FunPath;
...
//			((VRSoftGLView) mSufaceView).setType(XMVRType.XMVR_TYPE_SPE_CAM01);
	...
	private void switchSurfaceview(SDK_FishEyeFrameSW fishEyeFrame) {
		Log.i(TAG,"switchSurfaceview");
		if ( null == fishEyeFrame ) {
			...
			if ( null == mSufaceView
				...
				FecCenter fecCenter = new FecCenter(fishEyeFrame.st_5_imageWidth,
						fishEyeFrame.st_6_imageHeight,
						fishEyeFrame.st_2_centerOffsetX,
						fishEyeFrame.st_3_centerOffsetY,
						fishEyeFrame.st_4_radius);
				mSufaceView = new VRSoftGLView(getContext());

				if ( fishEyeFrame.st_1_lensType
	...
	public void setRealDevice(String devSn, int channel) {
		initSurfaceView();
		Log.i(TAG,"setRealDevice---"+devSn+"---channel"+channel);
		String playUrl = null;
		mChannel = channel;
	...
	private void openVideo() {
		...
		if ( mVideoUrl.startsWith("real://") ) {
			if ( !mIsPlaying ) {
				...
				Log.i(TAG,getUserId()+"---"+playPath+"---channel"+mChannel+"---"+mStreamType.getTypeId()+"---"+mSufaceView+"---"+0);
...
//            System.out.println("TTTT---->>" + deltaX);
//            System.out.println("TTTT---->>" + deltaY);
            if (count == 1) {
				if (deltime < 100) {
        ...
        case EUIMSG.ON_FRAME_USR_DATA:
            {
    			...
    			if ( null != fishFrame ) {
    				if (fishFrame instanceof SDK_FishEyeFrameSW) {
    					...
    					switchSurfaceview((SDK_FishEyeFrameSW)fishFrame);
    				}
    			}
            }
        	break;
